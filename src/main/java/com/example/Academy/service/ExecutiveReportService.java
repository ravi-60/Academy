...
public class ExecutiveReportService {
    ...
    public ExecutiveReportData getReportData(Long cohortId, LocalDate startDate, LocalDate endDate) {
        ...
        List<StakeholderEffort> efforts = effortRepository.findByCohortIdAndEffortDateBetween(cohortId, startDate,
                endDate);
        ...
        efforts.forEach(e -> {
            ExecutiveReportData.DailyReportLog.DailyReportLogBuilder builder = logMap.get(e.getEffortDate());
            if (builder != null) {
                switch (e.getRole()) {
